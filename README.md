...
View компоненты не хранят данные (нет полей для id, состояния и т.д.)

View компоненты не имеют геттеров (только сеттеры для отображения)

View компоненты не используют dataset для хранения данных

View компоненты получают обработчики через конструктор (actions объекты)

View компоненты не имеют прямого доступа к EventEmitter
...
Компоненты карточек:
Card - абстрактный базовый класс для карточек (только сеттеры, без геттеров)

CatalogCard - карточка товара в каталоге (принимает CatalogCardActions)

PreviewCard - детальный просмотр товара (принимает PreviewCardActions)

BasketCard - товар в корзине (принимает BasketCardActions, отдельный от Card)

Actions объекты
Для соблюдения принципа "View не хранят данные", обработчики передаются через объекты actions:

typescript
// Пример для BasketCard
interface BasketCardActions {
    onClick: () => void;
}

// Пример для CatalogCard
interface CatalogCardActions {
    onClick: () => void;
}

// Пример для PreviewCard
interface PreviewCardActions {
    onClick: () => void;
}

// В презентере (main.ts):
new BasketCard(element, {
    onClick: () => events.emit('basket:remove', { id: item.id })
});

new CatalogCard(element, {
...
Создание готовых элементов для View (BasketCard для корзины) с передачей actions объектов
...
Actions объекты - обработчики передаются в View через объекты actions

View без EventEmitter - карточки не имеют прямого доступа к EventEmitter

Нет dataset.id - данные не хранятся в DOM атрибутах
...
Пользователь просматривает каталог → Gallery отображает CatalogCard с actions объектом

Пользователь кликает на товар → CatalogCard вызывает action, отправляет card:select, Catalog вызывает preview:changed, открывается PreviewCard

Пользователь добавляет товар в корзину → PreviewCard вызывает action, отправляет preview:action, презентер решает добавить товар, Basket вызывает basket:changed

Пользователь открывает корзину → презентер создает BasketCard элементы с actions объектами и передает в BasketView
...
Actions через конструктор - обработчики передаются в View через конструктор

View без данных - представления не хранят бизнес-данные

Презентер создает связи - все связи между данными и обработчиками в презентере